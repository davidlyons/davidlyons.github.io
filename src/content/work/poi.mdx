...
description:
  ...
  of object rotations per arm rotation to create different numbers of in-spin and anti-spin petals.
tags:
  - Three.js
  - React
  - Material UI
...
```js
x = cos(angle * armTurns) * armLength + cos(angle * poiTurns) * poiLength * (startOut ? 1 : -1);
y = sin(angle * armTurns) * armLength + sin(angle * poiTurns) * poiLength * (startOut ? 1 : -1);
```

Where `angle` is in radians between `0` and `2π`
